...
fit_model_allo_ego <- function(pid, distfunc = euc_sim_list){
  d <- df_centered %>% 
    ...
    mutate(
      rweighted = (profile_rating_1 * sim_1 + profile_rating_2 * sim_2 + profile_rating_3 * sim_3 + profile_rating_4 * sim_4 + profile_rating_5 * sim_5 + profile_rating_6 * sim_6)/(sim_1 + sim_2 + sim_3 + sim_4 + sim_5 + sim_6),
      pweighted =  (recommender_rating * cos_sim)
    ) 
  
  return(lm(predicted ~ rweighted + pweighted, d))
}
...
fit_popego_allo <- function(pid, simfunc = euc_sim_list){
  ...
  return(lm(predicted ~ Average_Rating + recommender_rating + rweighted, d))
}
...
fit_pop_allo <- function(pid, simfunc = euc_sim_list){
  ...
  return(lm(predicted ~ rweighted + Average_Rating, d))
}
...
fit_pop_allo_neg <- function(pid, simfunc = euc_sim_list){
  ...
  fit <- nlsLM(predicted ~ v*Average_Rating + b + z * (profile_rating_1 * (sim_1 - a) + profile_rating_2 * (sim_2 - a) + profile_rating_3 * (sim_3 - a) + profile_rating_4 * (sim_4 - a) + profile_rating_5 * (sim_5 - a) + profile_rating_6 * (sim_6 - a) )/(sim_1 + sim_2 + sim_3 + sim_4 + sim_5 + sim_6), d, start=list(a=1, b=0, z = 1, v = 1), algorithm="port", 
    lower=c(0, -100, -100, -100), upper=c(2,100, 100, 100) )
...
fit_popego_allo_neg <- function(pid, simfunc = euc_sim_list){
  ...
  fit <- nlsLM(predicted ~ r*recommender_rating + v*Average_Rating + b + z * (profile_rating_1 * (sim_1 - a) + profile_rating_2 * (sim_2 - a) + profile_rating_3 * (sim_3 - a) + profile_rating_4 * (sim_4 - a) + profile_rating_5 * (sim_5 - a) + profile_rating_6 * (sim_6 - a) )/(sim_1 + sim_2 + sim_3 + sim_4 + sim_5 + sim_6), d, start=list(a=1, b=0, z = 1, r=1, v=1), algorithm="port", 
    lower=c(0, -100, -100,-100,-100), upper=c(2,100, 100,100,100) )
...
df_aic_fit <- df_centered %>% 
  ...
  mutate(
    ...
    allo_neg_aic = pmap_dbl(list(allo_neg_r2, TSS, n, 3), r2aic_c),  
    ...
    pop_allo_neg_aic = pmap_dbl(list(pop_allo_neg_r2, TSS, n, 4), r2aic_c),  
    ...
    popego_allo_neg_aic = pmap_dbl(list(popego_allo_neg_r2, TSS, n, 5), r2aic_c),  
  ...
  mutate(
    min_aic = pmap_dbl(list(ego_aic, allo_aic, allobest_aic, allo_neg_aic, popular_aic, popego_aic, popego_allo_aic, pop_allo_aic, pop_allo_neg_aic, pop_allo_best_aic, popego_allo_neg_aic, popego_allo_best_aic, popego_allo_bestunw_aic), min),
    best_model = case_when(
      min_aic == ego_aic ~ "ego",
...
get_num_params <- function(model_name) {
  model_params <- list(
    "ego_r2" = 2,               
    "allo_r2" = 2,              
    "allo_neg_r2" = 3,    # has an extra param, all with neg do       
    "allobest_r2" = 2,          
    "allo_minmax_r2" = 2,       
    "hybrid_r2" = 2,            
    "popular_r2" = 2,           
    ...
    "popego_allo_r2" = 4,       
    "pop_allo_r2" = 3,          
    "pop_allo_neg_r2" = 4, # this too      
    "pop_allo_best_r2" = 3,     
    "pop_allo_bestunw_r2" = 3,  
    "pop_allo_minmax_r2" = 3,   
    "popego_allo_neg_r2" = 5,   # and here
...
df_with_fits <- df_centered %>% 
  ...
  mutate(
    max_r2 = pmap_dbl(list(ego_r2,allo_r2, allobest_r2, allo_neg_r2,popular_r2, hybrid_r2, popego_r2, popego_allo_r2, pop_allo_r2, pop_allo_neg_r2, pop_allo_best_r2, popego_allo_neg_r2, popego_allo_best_r2, popego_allo_bestunw_r2), max),
    best_model = case_when(
      max_r2 == ego_r2 ~ "ego",
      max_r2 == allo_r2 ~ "allo",
      max_r2 == allo_neg_r2 ~ "allo_neg",
...
model_levels <- c(
  ...
  "Allocentric",
  "Popular + Allocentric",
  "Popular + Egocentric + Allocentric",
  "Egocentric",
  "Popular",
  "Popular + Egocentric"
)
...
df_with_fits %>% 
  pivot_longer(c(contains("_r2"), -"max_r2"), names_to = "model", values_to = "R2") %>% 
mutate(model_grouped = case_when(
      model %in% c("allo_r2", "allo_neg_r2", "allo_minmax_r2", "allobest_r2") ~ "Allocentric",
      model %in% c("pop_allo_r2", "pop_allo_best_r2", "pop_allo_neg_r2", "pop_allo_bestunw_r2", "pop_allo_minmax_r2") ~ "Popular + Allocentric",
      model %in% c("popego_allo_r2","popego_allo_best_r2", "popego_allo_bestunw_r2", "popego_allo_neg_r2") ~ "Popular + Egocentric + Allocentric",
      model == "popego_r2" ~ "Popular + Egocentric",
  ...
  group_by(participantID, profile, cos_sim, euc_dist, mae, corr) %>% 
  mutate(
    delta_aic = aic - min(aic),
    w_aic = exp(-1/2 * delta_aic) / sum(exp(-1/2 * delta_aic)),
    hybrid_waic = sum(if_else(model_grouped == "Allocentric + Egocentric", w_aic, 0)),
    ego_waic = sum(if_else(model_grouped == "Allocentric", w_aic, 0)),
    max_waic = map_dbl(w_aic, max)) %>% 
  group_by(participantID, profile, hybrid_waic, ego_waic, max_waic, model_grouped) %>% 
  summarize(w_aic = mean(w_aic)) %>%  # equally weight prior according to overall grouping
  ungroup() %>% 
  mutate(
    model_grouped = factor(model_grouped, levels = model_levels)
  ) -> xx


# Assign numeric codes to each model
model_codes <- data.frame(
  model_grouped = model_levels,
  model_code = c(1, 2, 3, 4, 5, 6, 7)
)

model_codes_list = model_codes$model_code
names(model_codes_list) = model_codes$model_grouped
xx %>%
  left_join(model_codes, by = "model_grouped") %>%
  group_by(participantID) %>%
  mutate(
    # delta_aic = aic - min(aic),
    # w_aic = exp(-0.5 * delta_aic) / sum(exp(-0.5 * delta_aic)),
    best_model = first(if_else(w_aic == max(w_aic), model_grouped, NA), na_rm=TRUE),
    best_model_code = model_codes_list[best_model],
    model_order = model_codes_list[model_grouped]
  ) %>%
  ungroup() %>% 

# Now, use best_model_code to reorder participants in your plot
ggplot( aes(x = reorder(participantID, best_model_code - hybrid_waic), y = w_aic, fill = model_grouped, order = model_order)) +
  geom_col(position = "stack", width = 0.8) +

# xx %>% 
  # ggplot(aes(x = reorder(participantID, best_model_code ), y = w_aic, fill = model_grouped )) +
  # geom_col(position = "stack", width = 0.8) +  # Increase bar width for better visibility
  scale_fill_brewer(palette = "Set2") +  # Use a distinct color palette
  theme_bw() +
  theme(
    ...
    legend.text = element_text(size = 8)) +  
  labs(x = "Participant ID", y = "AIC Weight", fill = "Model Group") +
  ggtitle("AICc Weight Distribution by Participant ID")
```
...
df_aic_scatter <- df_aic_scatter %>%
  mutate(quadrant = case_when(
    ...
    corr <= 0.3 & best_model_r2 > 0.3 ~ "Poor Performance and Good Model Fit", 
    corr <= 0.3 & best_model_r2 <= 0.3 ~ "Poor Performance and Poor Model Fit" 
  ))
...
ggplot(df_aic_scatter, mapping = aes(x = corr, y = best_model_r2, color = quadrant)) +
  ...
  scale_color_manual(values = c(
    "Good Performance and Good Model Fit" = "darkgreen",
    "Good Performance and Poor Model Fit" = "red",
    "Poor Performance and Good Model Fit" = "darkgreen",
    "Poor Performance and Poor Model Fit" = "orange"
  )) +
  labs(x = "Performance at Inference Task", y = "How Well the Best Model Fit", color = "Quadrant") +
