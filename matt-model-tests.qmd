...
ggplot(df_scatter, aes(x = participantID, y = corr)) +
  geom_point() +
  labs(x = "Participant ID", y = "Correlation (Corr)", title = "Accuracies for the Best 10%, Average, and Worst 10% Participants") +
  facet_wrap(~ group) +  
  theme_minimal() +
  theme(
...
chunk3_data <- df_centered %>%
  ...
  mutate(alg = "Top 20% of Participants")
...
ggplot(combined_data, aes(x = alg, y = val)) +
  geom_boxplot() +
  coord_flip() +
  labs(y = "MAE", x = "Recommender", title = "Recommender Systems and Human Participants")
...
df_with_fits %>% 
  mutate(
    allo_summary_r2 = pmap_dbl(list(allo_r2, allobest_r2, allo_neg_r2, allo_minmax_r2), max)
  ) %>% 
  ...
  ggplot(aes(x=reorder(participantID, hybrid_waic + ego_waic), y = w_aic, fill = model)) +
  geom_col() +
  theme_bw() +
  theme(
  ...
  labs(x = "Participant ID", y = "AIC Weight", fill = "Model")
...
# Combined R² and AIC calculation
df_aic_fit <- df_centered %>% 
  group_by(participantID, profile, cos_sim, euc_dist) %>% 
  summarize(
    mae = mean(abs(predicted - profile_rating)),
    corr = cor(predicted, profile_rating),
    TSS = var(predicted) * n()  # Calculate TSS (Total Sum of Squares)
  ) %>% 
  ungroup() %>% 
  arrange(participantID) %>% 
  mutate(
    # R² calculations for each model
    ego_r2 = map_dbl(models_ego, get_R2),
    allo_r2 = map_dbl(models_allo, get_R2),
    allo_neg_r2 = map_dbl(models_allo_neg, get_R2_nls),
    allobest_r2 = map_dbl(models_allobest_unweighted, get_R2),
    ...
    # Dynamic calculation of n (group size) for each participant
    n = n(),

    # AIC calculations using r2aic_c function for each model
    ego_aic = pmap_dbl(list(ego_r2, TSS, n, 2), r2aic_c),  # 2 parameters for ego_r2
    allo_aic = pmap_dbl(list(allo_r2, TSS, n, 2), r2aic_c),  # 3 parameters for allo_r2
    allo_neg_aic = pmap_dbl(list(allo_neg_r2, TSS, n, 2), r2aic_c),  # 4 parameters for allo_neg_r2
    allobest_aic = pmap_dbl(list(allobest_r2, TSS, n, 2), r2aic_c),  # 3 parameters for allobest_r2
    allo_minmax_aic = pmap_dbl(list(allo_minmax_r2, TSS, n, 2), r2aic_c),  # 4 parameters for allo_minmax_r2
    hybrid_aic = pmap_dbl(list(hybrid_r2, TSS, n, 3), r2aic_c),  # 5 parameters for hybrid_r2
    popular_aic = pmap_dbl(list(popular_r2, TSS, n, 2), r2aic_c),  # 2 parameters for popular_r2
    popego_aic = pmap_dbl(list(popego_r2, TSS, n, 3), r2aic_c),  # 3 parameters for popego_r2
    popego_allo_aic = pmap_dbl(list(popego_allo_r2, TSS, n, 4), r2aic_c),  # 4 parameters for popego_allo_r2
    pop_allo_aic = pmap_dbl(list(pop_allo_r2, TSS, n, 3), r2aic_c),  # 4 parameters for pop_allo_r2
    pop_allo_neg_aic = pmap_dbl(list(pop_allo_neg_r2, TSS, n, 3), r2aic_c),  # 5 parameters for pop_allo_neg_r2
    pop_allo_best_aic = pmap_dbl(list(pop_allo_best_r2, TSS, n, 3), r2aic_c),  # 4 parameters for pop_allo_best_r2
    pop_allo_bestunw_aic = pmap_dbl(list(pop_allo_bestunw_r2, TSS, n, 3), r2aic_c),  # 4 parameters for pop_allo_bestunw_r2
    pop_allo_minmax_aic = pmap_dbl(list(pop_allo_minmax_r2, TSS, n, 3), r2aic_c),  # 5 parameters for pop_allo_minmax_r2
    popego_allo_neg_aic = pmap_dbl(list(popego_allo_neg_r2, TSS, n, 4), r2aic_c),  # 5 parameters for popego_allo_neg_r2
    popego_allo_best_aic = pmap_dbl(list(popego_allo_best_r2, TSS, n, 4), r2aic_c),  # 4 parameters for popego_allo_best_r2
    popego_allo_bestunw_aic = pmap_dbl(list(popego_allo_bestunw_r2, TSS, n, 4), r2aic_c)  # 4 parameters for popego_allo_bestunw_r2
  ) %>% 
...
# Count the best models based on AIC
df_aic_fit %>% 
  count(best_model)
