...
df_with_fits %>% 
  ...
  # filter(model %in% c("ego_r2", "allo_summary_r2", "hybrid_r2")) %>% 
  mutate(num_params = 2 ) %>% 
  mutate(
    aic = r2aic_c(R2, TSS, N, num_params)
    ) %>% 
...
- Random chance: [matt]
  - Compare correlation of cor(Ps estimates, profile ratings) vs. cor(model pred estimates, profile ratings) -- How many Ps are doing significantly better at predicting the profiles than we predict they would based on our best-fitting model?
