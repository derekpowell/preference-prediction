...
full_target_df <- full_target_df%>%
  select(participantID, Artwork_ID, Rating, Title, Average_Rating)
```
...
exclude_participants <- rating_proportions %>%
  filter(Proportion_Extreme > 0.75) %>%
  pull(participantID)
...
full_target_df2 <- full_target_df2 %>%
  mutate(Eccentricity_Category = case_when(
    Eccentricity_Score >= 0 & Eccentricity_Score < 0.05 ~ "0 - 0.05",
    Eccentricity_Score >= 0.05 & Eccentricity_Score < 0.1 ~ "0.05 - 0.1",
    Eccentricity_Score >= 0.1 & Eccentricity_Score < 0.15 ~ "0.1 - 0.15",
    Eccentricity_Score >= 0.15 & Eccentricity_Score < 0.2 ~ "0.15 - 0.2",
    Eccentricity_Score >= 0.2 ~ "0.2+",
    TRUE ~ "NA"
  ))
...
target_profiles <- full_target_df2%>%
  filter(participantID %in% c(160, 99, 235, 167))
