...
ratings_data <- as(recomm_data, "realRatingMatrix")
recommender_model <- Recommender(ratings_data, method = "UBCF")
```
...
recommendations <- predict(recommender_model, ratings_data[1:5], n = 5)
...
```{r}
evaluation_scheme <- evaluationScheme(ratings_data, method = "cross-validation", k = 5, given = 3, goodRating = 5)

results <- evaluate(evaluation_scheme, method = "UBCF", n = 5)
print(avg(results@results))
```
